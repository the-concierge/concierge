...
//     if (error) {
//       containerObservable.streaming = false
//     }
...
function hostEvent(host: Concierge.Host) {
  let predicate = (h => h.id === host.id)
  return function (error?: any, hostEvent?: string) {
    let hostObservable = info.hosts.find(predicate)

    if (error) {
      hostObservable.streaming = false
    } else {
      hostObservable.streaming = true
    }

    try {
      let event: HostEvent = JSON.parse(hostEvent)
      let container = info.containers.find(c => c.dockerId === event.id)
