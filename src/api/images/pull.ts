...
const handler: RequestHandler = async (req, res) => {
  ...
  if (hosts.length === 0) {
    return res.status(412).json({ message: 'No hosts are available to service this request' })
  }

  if (typeof imageName !== 'string' || typeof tag !== 'string') {
    return res.status(400).json({ message: 'Invalid image name or tag' })
  }
  ...
  try {
    ...
    return res.status(500).json({ message: ex.message || ex })
  }
}
