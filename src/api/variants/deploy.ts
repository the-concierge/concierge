...
export default async function deploy(application: Concierge.Application, tag: string) {
  ...
  if (isAlreadyDeployed) {
    await saveVariant(application.id, tag, DeployedState.Deployed)
    return true
  }
...
async function deployImage(application: Concierge.Application, tag: string) {
  ...
  await saveVariant(application.id, tag)[0]
  try {
    let buildResult = await buildImage(application, tag)
    let pushResult = await push(tag, application, buildResult.host)
