 ...
 */
export default function inject(container: Concierge.Container, volumeData: Buffer) {
	if (!container.host) {
	...
	putTar(host, tarFilename, volumeData);

	// Execute the unpack command
	unpackTar(host, destination, tarFilename);

	// Remove the evidence
	removeTar(host, tarFilename);
	return true;	
}

function putTar(host: Concierge.Host, filename: string, tar: Buffer) {
	const result = await(writeFile(host, filename, tar));
	return result;
}

function unpackTar(host: Concierge.Host, destination: string, filename: string) {	
...
function removeTar(host: Concierge.Host, filename: string) {
