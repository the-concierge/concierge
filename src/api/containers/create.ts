...
import * as getContainerInfo from './getInfo';
...
function getContainerHost(request: Concierge.NewContainer) {
    if (request.host) return getHost.one(request.host);
    return getLeastLoadedHost();
}

function saveContainer(container: Concierge.Container, containerType: NewContainerType) {
    var isChange = containerType === NewContainerType.Change;
    if (isChange) return Promise.resolve(container);
...
function isValidRequest(request: Concierge.NewContainer) {
    ...
    var isValidVariant = !!request.variant;
    var isValidLabel = !!request.label && request.label.length > 0;

    var isValid = isValidVariant && isValidLabel;
